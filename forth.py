...
peforth.dictate('''
    ...
    \ ------ breakpoint ------------------------------------------------------------
    : bp ." Usage: peforth.bp(11,locals())  # drop a breakpoint with ID=11" cr ; // Usage: peforth.bp(11,locals()) # drop a breakpoint with ID=11
    ...
    char https://app.datarobot.com/api/v2 constant endpoint // ( -- url ) DataRobot's endpoint URL
    ...
    ' d :: name='dump2' rescan-word-hash \ rename the 'd' dump to avoid problems
    ' *debug* :> comment
    ' *debug* ::~ comment+="Example: %f ( %f true to debug ) debug [if] *debug* [then] ( false to debug exit )" 
    ''')
